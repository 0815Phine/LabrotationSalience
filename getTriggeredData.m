...
for t = 1:length(Triggers)
    ...
    for cu = 1:length(Spikes)
        spks = Spikes{cu};
        sp_spks = cell(length(triggers), 1);
        ev_spks = cell(length(triggers), 1);
        for ct = 1:length(triggers)
            sp_spks{ct} = spks(spks > spont_window(ct,1) & spks < spont_window(ct,2))-triggers(ct); % relative spks
            ev_spks{ct} = spks(spks > ev_window(ct,1) & spks < ev_window(ct,2))-triggers(ct); % relative spks
        end
    ...
    relSpks = cell(size(consSpks));
    burstspkinds = cell(size(consSpks));
    ...
    for cu = 1:length(Spikes)
        ...
        for ct = 1:2
            relSpks{cu,ct} = cell(size(consSpks{cu,ct}));
            burstspkinds{cu,ct} = cell(size(consSpks{cu,ct}));

            for i = 1:length(consSpks{cu,ct})
                spks = consSpks{cu,ct}{i};
                ...
                if ~isempty(spks)
                    ...
                    isis = abs(diff(spks));
                    burstSpkInds = find(isis<ISIcutoff);
                    burstSpkInds = unique([burstSpkInds; burstSpkInds+1]);

                    relSpks{cu, ct}{i} = spks;
                    burstspkinds{cu,ct}{i} = burstSpkInds;
...
%                     end
    ...
    TriggeredData(t).ConditionName = conscondnames{t};
    TriggeredData(t).burstCF = burstCF;
    TriggeredData(t).bursteventRatio = bursteventRatio;
    TriggeredData(t).nBursts = nBursts;
    TriggeredData(t).nSpikes = nSpikes;
    TriggeredData(t).relSpks = relSpks;
    TriggeredData(t).burstSpkInds = burstspkinds;
